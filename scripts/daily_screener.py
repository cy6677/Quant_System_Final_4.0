...
# ========= 你可以改呢兩個開關 =========
ENABLE_IBKR_EXECUTION = False   # 冇開 TWS/Paper 就保持 False
IBKR_PORT = 7496                # TWS 常用 7496，Paper 常用 7497
# ====================================


def get_ib_connection(port: int = IBKR_PORT):
    try:
    ...
    try:
        ...
        print(f"✅ 成功連接 IBKR! (port={port})")
        return ib
    except Exception as e:
        print(f"⚠️ 無法連接 IBKR (TWS/Paper 可能未開): {e}")
        return None
...
def run_screener():
    ...
    ib = get_ib_connection(port=IBKR_PORT)
    ...
    print(f"   📌 Swing 總分配: ${capital_swing_total:,.2f} (每策略 ${capital_swing_each:,.2f})")
    ...
    if not ENABLE_IBKR_EXECUTION:
        print("⏸️ 已停用 IBKR 自動落單（ENABLE_IBKR_EXECUTION=False）")
        print("📌 你未開 TWS/Paper 時，呢個係預期行為。")
        return
